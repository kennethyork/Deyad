...
> **Important Note**: This is a fork of the [original OpenAI Codex CLI](https://github.com/openai/codex) with a simplified Ollama-only backend and changed installation instructions. The main differences in this fork are:
>
> - Support for only the Ollama provider
> - Uses the [Chat Completion API instead of the Responses API](https://platform.openai.com/docs/guides/responses-vs-chat-completions) with an Ollama-compatible endpoint
> - All other functionality remains similar to the original project
> - You can install this fork globally with `npm i -g deyad-cli`
...
Next, ensure your Ollama server is running and optionally set a custom base URL:

```shell
export OLLAMA_BASE_URL="http://localhost:11434/v1"
```

> **Note:** Ollama typically runs locally, so no API key is required. If you use a different host or port, set `OLLAMA_BASE_URL` accordingly.
>
> **Tip:** You can also place your environment variable into a `.env` file at the root of your project:
>
> ```env
...
```shell
deyad-cli "explain this codebase to me"
```

```shell
deyad-cli --approval-mode full-auto "create the fanciest todo-list app"
```
...
- **Zero setup** — no API key is required for local Ollama use
- **Ollama-only backend** — this fork is designed for Ollama models exclusively
...
- **Linux** – there is no sandboxing by default.
  ...
  root on the host. You can use the [`run_in_container.sh`](./deyad-cli/scripts/run_in_container.sh) script to set up the sandbox.
...
| ----------------------------------------- | ----------------------------------- | ------------------------------------ |
| `deyad-cli`                              | Interactive REPL                    | `codex`                              |
| `deyad-cli "…"`                          | Initial prompt for interactive REPL | `codex "fix lint errors"`            |
| `deyad-cli -q "…"`                       | Non‑interactive "quiet mode"        | `codex -q --json "explain utils.ts"` |
| `deyad-cli completion <bash\|zsh\|fish>` | Print shell completion script       | `codex completion bash`              |
...
- name: Update changelog via Codex
  run: |
    ...
    deyad-cli -a auto-edit --quiet "update CHANGELOG for next release"
```
...
Below are a few bite‑size examples you can copy‑paste. Replace the text in quotes with your own task. See the [prompting guide](https://github.com/openai/codex/blob/main/deyad-cli/examples/prompting_guide.md) for more tips and usage patterns.
...
git clone https://github.com/ymichael/deyad-cli.git
...
This is a fork of the original OpenAI Codex CLI project with expanded support for multiple AI providers beyond just OpenAI. The installation package is also different (`deyad-cli` instead of `@openai/codex`), but the core functionality remains similar.
...
To publish a new version of the CLI, run the release scripts defined in `deyad-cli/package.json`:

1. Open the `deyad-cli` directory
...
4. Commit the version bump (with DCO sign-off):
   ```bash
   git add deyad-cli/src/utils/session.ts deyad-cli/package.json
   git commit -s -m "chore(release): deyad-cli v$(node -p \"require('./deyad-cli/package.json').version\")"
   ```
