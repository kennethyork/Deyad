...
/**
 * Creates a .log file for this session, but also symlinks deyad-cli-latest.log
 * to the current log file so you can reliably run:
 *
 * - Mac/Windows: `tail -F "$TMPDIR/oai-codex/deyad-cli-latest.log"`
 * - Linux: `tail -F ~/.local/oai-codex/deyad-cli-latest.log`
 */
export function initLogger(): Logger {
  ...
  const logFile = path.join(logDir, `deyad-cli-${now()}.log`);
  // Write the empty string so the file exists and can be tail'd.
  fsSync.writeFileSync(logFile, "");

  // Symlink to deyad-cli-latest.log on UNIX because Windows is funny about
  // symlinks.
  if (!isWin) {
    const latestLink = path.join(logDir, "deyad-cli-latest.log");
    try {
