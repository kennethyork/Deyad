...
export async function runOnce(
  ...
  const callbacks: AgentCallbacks = {
    ...
    onToolStart: (name, params) => {
      ...
      spinner.start(); spinnerActive = true;
    },
    onToolResult: (r) => {
      if (spinnerActive) { spinner.stop(); spinnerActive = false; }
  ...
  if (args.auto && args.prompt) {
    ...
    const autoCallbacks: AgentCallbacks = {
      ...
      onToolStart: (name, params) => {
        if (autoSpinnerActive) { autoSpinner.stop(); autoSpinnerActive = false; }
        console.log('\n' + formatToolStart(name, params));
        autoToolStartTime = Date.now();
        autoSpinner.update(`Running ${name}...`);
        autoSpinner.start(); autoSpinnerActive = true;
      },
      onToolResult: (r) => {
        if (autoSpinnerActive) { autoSpinner.stop(); autoSpinnerActive = false; }
        const elapsed = autoToolStartTime ? ` ${((Date.now() - autoToolStartTime) / 1000).toFixed(1)}s` : '';
        console.log(formatToolEnd(r.tool, r.success, r.output, elapsed));
...
`;
      ...
      const callbacks: AgentCallbacks = {
        ...
        onToolStart: (name, params) => {
          ...
          replToolStartTime = Date.now();
          replSpinner.update(`Running ${name}...`);
          replSpinner.start(); replSpinnerActive = true;
        },
        onToolResult: (r) => {
          if (replSpinnerActive) { replSpinner.stop(); replSpinnerActive = false; }
          const elapsed = replToolStartTime ? ` ${((Date.now() - replToolStartTime) / 1000).toFixed(1)}s` : '';
          console.log(formatToolEnd(r.tool, r.success, r.output, elapsed));
        },
        onDiff: (path, diff) => {
          if (replSpinnerActive) { replSpinner.stop(); replSpinnerActive = false; }
