...
Deyad runs [Ollama](https://ollama.ai) on your machine for AI inference, scaffolds real production stacks (React + Express + SQLite + Prisma), and gives you a complete IDE with live preview, terminal, database admin, version history, and one-click deployment.
...
| Built-in database admin (Prisma Studio) | **Yes** | No | No | No | No | No |
...
- **Prisma Studio** — embedded database admin UI inside the app
- **One-click start/stop** — launch Prisma Studio from the UI
...
| [Docker](https://docker.com) or [Podman](https://podman.io) | Container-based deploys | Optional |
...
5. **Database** — toggle the DB on to launch Prisma Studio for your SQLite database
...
│   ├── prisma/
│   └── package.json
├── backend/prisma/     # Prisma schema + SQLite DB
