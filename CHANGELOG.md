...
- Replaced `uuid` dependency with native `crypto.randomUUID()` (14 → 13 runtime deps)
- Test-to-source ratio raised from 0.70 to 1.0 (12,412 test lines / 12,582 source lines)
- Expanded 16 test files with 4,166 new lines of test coverage

### Fixed

- Agent loop crash when `executeTool` throws — now catches and returns error result
- Unhandled promise rejection in `loadApps` when `listApps()` fails
- Unhandled promise rejection in `DeployModal` when `deployCheck()` fails
- Silent success for unknown deploy providers — now returns `success: false`
