...
export default function SettingsModal({ onClose }: Props) {
  ...
  const handleSave = async () => {
    setSaving(true);
    setSaved(false);
    await window.deyad.setSettings({ ollamaHost: ollamaHost.trim(), defaultModel });
    setSaving(false);
    setSaved(true);
  ...
  const handleTest = async () => {
    ...
    try {
      // Save the current host first so the backend uses it
      await window.deyad.setSettings({ ollamaHost: ollamaHost.trim() });
      const { models: list } = await window.deyad.listModels();
      setModels(list.map((m) => m.name));
